...
exports.homepage = async (req, res) => {
  try {
    ...
    const nepalese = await Recipe.find({ category: "Nepalese" }).limit(
      limitNumber
    );

    const food = { latest, newari, nepalesetibetian, nepalese };

    res.render("index", { title: "Nepali cusine - Home", categories, food });
  } catch (error) {
...
exports.exploreRecipe = async (req, res) => {
  try {
    ...
    res.render("recipe", { title: "Nepali cusine - Recipe", recipe });
  } catch (error) {
    res.status(500).send({ message: error.message || "Error Occured" });
  }
};
...
exports.searchRecipe = async (req, res) => {
  ...
  try {
    ...
    res.render("search", { title: "Nepali cusine - Search", recipe });
  } catch (error) {
    res.status(500).send({ message: error.message || "Error Occured" });
  }
};
